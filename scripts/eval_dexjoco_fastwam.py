...
def _save_episode_video(frames: list[np.ndarray], path: Path, fps: int) -> None:
    if Mp4VideoWriter is None:
        raise RuntimeError(
            "Video saving requested, but dexjoco.data.video_writer is unavailable. "
            "Run with --no-save-video or install the DexJoCo video writer helper."
        )
