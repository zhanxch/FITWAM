...
_WORKER_REQUEST_QUEUE: mp.Queue | None = None
_WORKER_RESPONSE_QUEUE: mp.Queue | None = None


def _init_worker_queues(request_queue: mp.Queue, response_queue: mp.Queue) -> None:
    global _WORKER_REQUEST_QUEUE, _WORKER_RESPONSE_QUEUE
    _WORKER_REQUEST_QUEUE = request_queue
    _WORKER_RESPONSE_QUEUE = response_queue
...
def _normalize_video_frames(frames: list[np.ndarray]) -> list[np.ndarray]:
    out: list[np.ndarray] = []
    for frame in frames:
        ...
        if arr.ndim != 3 or arr.shape[-1] not in (3, 4):
            raise ValueError(f"video frame must be HxWx3/4, got {arr.shape}")
        if arr.shape[-1] == 4:
            arr = arr[..., :3]
...
def _save_episode_video(frames: list[np.ndarray], path: Path, fps: int) -> None:
    ...
    frames_u8 = _normalize_video_frames(frames)
    if Mp4VideoWriter is not None:
    ...
    try:
        ...
        imageio.mimsave(str(path), frames_u8, fps=fps, macro_block_size=1)
        return
    except Exception as imageio_exc:
        try:
            import cv2

            height, width = frames_u8[0].shape[:2]
            ...
            raise RuntimeError(
                "Video saving requested, but no available writer succeeded: "
                f"imageio={imageio_exc!r}; cv2={cv2_exc!r}"
            ) from cv2_exc
            ...
            try:
                ...
                if not isinstance(payload, dict) or "actions" not in payload:
                    if isinstance(payload, dict):
                        detail = f"keys={sorted(payload.keys())}"
                    else:
                        detail = f"type={type(payload).__name__}"
                    raise RuntimeError(
                        "Unexpected get_actions_batch response: "
                        f"{detail}; payload={repr(payload)[:500]}"
                    )
...
def _episode_worker(job: dict[str, Any]) -> dict[str, Any]:
    ...
    request_queue = _WORKER_REQUEST_QUEUE
    response_queue = _WORKER_RESPONSE_QUEUE
    policy = _PolicyFacade(
    ...
    try:
        if request_queue is None and not policy.ping():
            raise RuntimeError(
...
def evaluate_task_async(
    ...
    *,
    ...
    try:
        ...
        if use_batch_infer:
            pool_kwargs = {
                "initializer": _init_worker_queues,
                "initargs": (request_queue, response_queue),
            }
        with ctx.Pool(processes=max(1, batch_size), **pool_kwargs) as pool:
