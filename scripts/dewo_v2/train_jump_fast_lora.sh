#!/usr/bin/env bash
# Internal DEWO v2 tmux worker (kept for existing fold_glasses wrappers).
# New launches: TASK=... INIT=scratch|s0|lora GPUS=... ENV_FILE=... bash scripts/dewo_v2/train.sh
#
#   source data/<task>_dewo_v2_pair_*/eve_v02/protocol/offline_v1_b1_jump_fast.env
...
# Default: online VAE / no pre-encode. Opt-in: USE_VAE_LATENT_CACHE=1.
...
if [[ -f "${ENV_FILE}" && "${USE_VAE_LATENT_CACHE:-0}" == "1" ]]; then
...
export DEWO_TASK="${DEWO_TASK:-dexjoco/dexjoco_dewo_v2_offline_b1_jump_fast_lora_3e-5}"
...
export USE_VAE_LATENT_CACHE="${USE_VAE_LATENT_CACHE:-0}"
...
if [[ "${RUN_INLINE}" == "1" ]]; then
  echo "[dewo-v2-train] running train inline (use_cache=${USE_VAE_LATENT_CACHE:-0} fill=${FILL_VAE_LATENT_CACHE} skip_vae_preencode=${SKIP_VAE_PREENCODE})"
  exec bash "${WORKER}"
fi
...
echo "[dewo-v2-train] VAE: use_cache=${USE_VAE_LATENT_CACHE:-0} cache=${VAE_LATENT_CACHE_DIR:-<none>} skip_preencode=${SKIP_VAE_PREENCODE}"
