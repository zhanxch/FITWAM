...
\title{InterWAM: Interaction-Centric World Action Models\\for Physical Intent and Consequence Understanding}
...
\begin{abstract}
World Action Models (WAMs) jointly predict future observations and actions, offering a promising path toward generalist robot policies.
Existing WAMs are predominantly trained on \emph{successful} demonstration trajectories with vision as the primary sensory channel.
We argue that this recipe yields models that imitate motion patterns without grounding in \emph{physical interactions}: what contact is intended, whether it succeeded, and what consequences follow from a failed or partial interaction.
We propose \textbf{InterWAM}, an \textbf{interaction-centric} WAM that reorganizes learning around \emph{interaction events} rather than full task episodes.
Each event is a temporally localized segment defined by an agent--object interaction (e.g., grasp, place, push) with explicit intent labels.
We further incorporate \textbf{tactile feedback} as a contact-centric modality and \textbf{failure trajectories} as supervision for interaction outcomes.
InterWAM extends the mixture-of-transformers (MoT) architecture of FastWAM with a tactile expert and event-conditioned training objectives that disentangle \emph{interaction intent} from \emph{interaction consequence}.
On DexJoCo bimanual microwave manipulation, we show that event-centric data construction improves subtask alignment, tactile fusion sharpens contact-phase prediction, and failure-aware training reduces compounding errors after mispredicted interactions.
\end{abstract}
