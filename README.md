...
| **M3 Steer token** | 从局部成功/失败动作学习 observation-conditioned residual steer | steer token 本体尚未实现；现有 `outcome_flag` 不能作为主方法 |
...
Steer token 本体还没实现。当前代码里只有 outcome plumbing：
...
当前默认配置里 `model.outcome_num_classes: 0`。M3 不直接打开这条路径，因为真实 outcome 进入 shared context 会造成训练/推理接口不一致。目标是：
...
3. M3：实现 offline contrastive steer，包括 trajectory Teacher、observation Student、residual action-side steer、训练配置和闭环评测。
